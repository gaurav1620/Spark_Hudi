...
object StreamHandler {
    ...
    println("HELLOOOOOOOOOOOOOOOOOOO WORLD")
        ...
        //Filter queries
          osquery_names.foreach(query_name =>{
           ...
           outLoop.breakable{
            ...
            val newfg = filteredDF.
              select(from_json(col("value"), schema).alias("value")).select("value.*")

            println("*********************************************")
            println(schema.getClass)
            //filteredDF.show(2,false)
            //newfg.select("columns").show(2,false)
            //newfg.show(2,false)

            //val schema2 = newfg.select(schema_of_json(newfg.select(col("columns")).first.getString(0))).as[String].first
            val replacedString = newfg.select(col("columns")).first.getString(0).replaceAll("\"\"","\"temp\"")

            //val json_schema = spark.read.json(newfg.rdd.map(lambda row: row.columns)).schema
            //println(json_schema)
            println("##################")
            ...
            val df99 = newfg.selectExpr(expr_string).select("parsed_json.*")
            df99.
              write.
...
//              option("hoodie.upsert.shuffle.parallelism","1").
            ...
            //val anotherOne = newfg.withColumn('new_json_column', from_json(col('columns'), json_schema))

           // val totallynew = newfg.select(from_json(col("columns"), schema2).alias("columns")).select("columns.*")
           //val totallynew = newfg.select("columns.*")

            //totallynew.show(2,false)

            //println(newfg.schema)
              //show(2,false)
